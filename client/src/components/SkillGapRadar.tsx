import React from 'react';
import {
  BarChart,
  Bar,
  XAxis,
  YAxis,
  Tooltip,
  ResponsiveContainer,
  Legend,
  CartesianGrid,
} from 'recharts';
import { GapItem } from '../types';
import { useTheme } from '../context/ThemeContext';

interface Props {
  breakdown: GapItem[];
}

export const SkillGapRadar: React.FC<Props> = ({ breakdown }) => {
  const { theme } = useTheme();

  if (!breakdown || breakdown.length === 0) {
    return (
      <div className="p-8 text-center text-textSecondary text-sm">
        No competency profile data available. Complete self-assessment to view skill gap analytics.
      </div>
    );
  }

  // Dynamic colors derived from theme so Recharts updates instantly on theme switch
  const primaryColor = theme === 'dark' ? '#4ADE80' : '#166534';
  const accentColor = theme === 'dark' ? '#F59E0B' : '#B45309';
  const borderColor = theme === 'dark' ? '#3A342D' : '#E8E0D4';
  const textMutedColor = theme === 'dark' ? '#A8A095' : '#78716C';

  const chartData = breakdown.map((item) => ({
    name: item.competencyName.length > 24 ? `${item.competencyName.slice(0, 22)}...` : item.competencyName,
    fullName: item.competencyName,
    'Current Level': item.currentLevel,
    'Required Level': item.requiredLevel,
    'Skill Gap': item.gap,
  }));

  return (
    <div className="w-full">
      <div className="h-64 sm:h-72">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
            data={chartData}
            margin={{ top: 20, right: 30, left: 0, bottom: 25 }}
          >
            <CartesianGrid strokeDasharray="3 3" stroke={borderColor} />
            <XAxis
              dataKey="name"
              stroke={textMutedColor}
              fontSize={11}
              interval={0}
              angle={-15}
              textAnchor="end"
            />
            <YAxis
              domain={[0, 5]}
              ticks={[1, 2, 3, 4, 5]}
              stroke={textMutedColor}
              fontSize={12}
            />
            <Tooltip
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  const data = payload[0].payload;
                  return (
                    <div className="bg-surface text-textPrimary p-3 rounded-xl shadow-paper-lg text-xs border border-border">
                      <p className="font-bold text-primary mb-1">{data.fullName}</p>
                      <p className="text-textSecondary">
                        Current Level: <span className="font-bold text-textPrimary">{data['Current Level']} / 5</span>
                      </p>
                      <p className="text-textSecondary">
                        Required Benchmark: <span className="font-bold text-textPrimary">{data['Required Level']} / 5</span>
                      </p>
                      <p className="text-accent font-semibold mt-1">
                        Deficiency Gap: {data['Skill Gap']} levels
                      </p>
                    </div>
                  );
                }
                return null;
              }}
            />
            <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '10px', color: textMutedColor }} />
            <Bar dataKey="Current Level" fill={primaryColor} radius={[4, 4, 0, 0]} />
            <Bar dataKey="Required Level" fill={accentColor} radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>

      {/* Metric Pills */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-4">
        {breakdown.map((item) => (
          <div
            key={item.competencyId}
            className={`p-3 rounded-xl border flex flex-col justify-between ${
              item.gap === 0
                ? 'bg-surface border-primary/40'
                : item.gap >= 2
                ? 'bg-surface border-danger/40'
                : 'bg-surface border-accent/40'
            }`}
          >
            <div className="flex items-start justify-between gap-2">
              <span className="font-medium text-xs text-textPrimary line-clamp-1">{item.competencyName}</span>
              <span
                className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded border ${
                  item.gap === 0
                    ? 'bg-primarySoft text-primary border-primary/30'
                    : item.gap >= 2
                    ? 'bg-dangerSoft text-danger border-danger/30'
                    : 'bg-accentSoft text-accent border-accent/30'
                }`}
              >
                {item.gap === 0 ? 'Target Met' : `-${item.gap} Gap`}
              </span>
            </div>
            <div className="flex items-center justify-between text-[11px] text-textSecondary mt-2">
              <span>Level {item.currentLevel} of {item.requiredLevel}</span>
              <div className="w-20 bg-surface2 rounded-full h-1.5 overflow-hidden border border-border">
                <div
                  className={`h-full ${item.gap === 0 ? 'bg-primary' : 'bg-accent'}`}
                  style={{ width: `${Math.min(100, (item.currentLevel / item.requiredLevel) * 100)}%` }}
                />
              </div>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};
